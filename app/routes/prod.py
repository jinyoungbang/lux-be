...
def toggle_transaction(transaction_id):
    ...
    if success:
        return jsonify({"message": "Transaction 'is_hidden' field toggled successfully."})
    else:
...
def get_all_monthly_transactions():
    ...
    try:
        ...
        return jsonify({"error": "Invalid date format. Use the format: 'YYYY-MM-DD'"}, 400)
...
def get_last_6_monthly_spending():
    ...
    for i in range(6):
        ...
        monthly_transactions = [
            transaction for transaction in all_transactions if (
                start_date <= transaction["date"] <= end_date
                and transaction["amount"] > 0
                and ("is_hidden" not in transaction or ("is_hidden" in transaction and not transaction["is_hidden"]))
            )
        ]

        total_spending = sum(transaction.get("amount", 0) for transaction in monthly_transactions)
